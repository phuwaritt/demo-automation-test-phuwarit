...
android_emulator_name= ["Copy_of_Pixel_8_API_33", "Medium_Phone_API_35"]
...
def start_appium() {
    ...
    for(int i=0; i<APPIUM_PORT.size(); i++){
        ...
        try {
            sh "appium -p ${APPIUM_PORT[i]} --driver-xcuitest-webdriveragent-port ${WDA_PORT[i]} --relaxed-security &"
        }
        catch(err) {
